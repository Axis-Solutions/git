...
if(!empty($code) && !empty($stk_name) && !empty($TaxCode) && !empty($sellingPrice) && !empty($category))
{
    ...
    if(in_array($code, $codes)){
      $response["msg"] = "Product with code $code already exist in the system. No duplicate codes required!";
    $response["log"]="failed";   
    }
    else{
    if($expiry_date==""){
...
if($new_st["status"]=="ok"){
    $response["msg"] = "New Stocks have been successfully uploaded in the system";
    $response["log"]="ok";
}else{
    $response["msg"] = "Failed to create stock. Error: ".$new_st["status"];
    $response["log"]="failed";
}
}
}
else{
     $response["msg"] = "Product $code code, name, tax code, selling price and category can not be empty";
    $response["log"]="failed";
}
